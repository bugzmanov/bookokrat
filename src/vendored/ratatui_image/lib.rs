...
pub enum Resize {
    ...
    /// Viewport rendering
    ///
    /// Renders a viewport of the full image without cropping or re-encoding.
...
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
/// Specifies the viewport position when rendering a portion of an image.
...
impl Resize {
    ...
    pub fn needs_resize(
        ...
        if !force
            && !matches!(self, &Resize::Scale(_) | &Resize::Viewport(_))
    ...
    fn resize_image(&self, source: &ImageSource, width: u32, height: u32) -> DynamicImage {
        ...
        match self {
            ...
            Self::Viewport(options) => {
                // For Kitty protocol with tiling, the full image is handled separately
                // in StatefulProtocol::resize_encode, which directly uses source.image.
                // This path is for non-Kitty protocols that need a cropped viewport.

                // Crop from the viewport offset
                let x = options.x_offset.min(image.width().saturating_sub(width));
                let y = options.y_offset.min(image.height().saturating_sub(height));

                // Crop to the visible viewport area
                image.crop_imm(
                    x,
                    y,
